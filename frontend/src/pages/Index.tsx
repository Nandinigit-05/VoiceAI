...
import { Mic, Loader2, ServerOff } from "lucide-react";
import { analyzeAudio, AnalysisResponse, ConnectionError, APIError } from "@/services/api";
...
interface AnalysisData {
  ...
  ai_feedback: AnalysisResponse["ai_feedback"];
}
  ...
  const handleRecordingComplete = async (audioBlob: Blob, duration: number) => {
    ...
    try {
      ...
      if (err instanceof ConnectionError) {
        ...
        console.error(`[Error] Backend connection failed. API URL: ${err.apiUrl}`);
      } else if (err instanceof APIError) {
        // Handle API errors (server responded but with error)
        setError(`Server error (${err.status}): ${err.message}`);
        console.error(`[Error] API error. Status: ${err.status}, URL: ${err.apiUrl}`);
          ...
          <div className="flex flex-col items-center justify-center min-h-[70vh] px-4">
            <div className="glass-card p-12 w-full max-w-md text-center">
              ...
              <h2 className="font-display text-2xl font-semibold mb-2 text-destructive">
                {error.includes('Backend server is not running') ? 'Backend Not Available' : 'Analysis Failed'}
              </h2>
              ...
              {error.includes('Backend server is not running') && (
                <div className="mb-6 p-4 rounded-lg bg-secondary/50 border border-border">
                  <p className="text-sm text-muted-foreground text-left">
                    ...
                    <br />
                    1. Navigate to the <code className="text-xs bg-background px-1 py-0.5 rounded">backend</code> directory
                    <br />
                    2. Run: <code className="text-xs bg-background px-1 py-0.5 rounded">python app.py</code>
                    <br />
                    3. Ensure the server is running on <code className="text-xs bg-background px-1 py-0.5 rounded">http://127.0.0.1:5000</code>
                  </p>
                </div>
              )}
              <button
